...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(v_index):
            max_depth = 0
            for neighbor in self.get_adjacent_vertices(v_index):
                depth = 1 + dfs_depth(neighbor)
                max_depth = max(max_depth, depth)
            return max_depth

        for i, vertex in enumerate(self.vertices):
            vertex.depth = dfs_depth(i)


    def has_cycle(self):
        """
        Determine whether the graph has a cycle using DFS (styled like DFSGraph).
        Returns:
        ...
        def dfs_visit(vertex):
            vertex.color = "gray"
            for neighbor_index in self.get_adjacent_vertices(self.get_index(vertex.label)):
